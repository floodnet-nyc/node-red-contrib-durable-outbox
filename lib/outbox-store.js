  ...
  migrate() {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS outbox_pending_ready
        ON outbox_jobs (sink, available_at, created_at, id)
        WHERE state = 'pending';
  ...
  migrateQueueCounters() {
    this.transaction(() => {
      this.db.exec(`
        ...
        CREATE TRIGGER IF NOT EXISTS outbox_queue_count_insert
        ...
        CREATE TRIGGER IF NOT EXISTS outbox_queue_count_deliver
        ...
        CREATE TRIGGER IF NOT EXISTS outbox_queue_count_reactivate
        AFTER UPDATE OF state ON outbox_jobs
        WHEN OLD.state = 'delivered' AND NEW.state != 'delivered'
        BEGIN
          INSERT INTO outbox_queue_counts (sink, queued)
        ...
        CREATE TRIGGER IF NOT EXISTS outbox_queue_count_delete
