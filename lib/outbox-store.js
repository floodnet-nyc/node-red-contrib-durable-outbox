...
class OutboxStore {
  constructor(filename, options = {}) {
    ...
    this.deadLetterRetentionMs = Math.max(
      0,
      options.deadLetterRetentionMs == null
        ? D.DEAD_LETTER_RETENTION_MS
        : Number(options.deadLetterRetentionMs)
    );
  ...
  storageHealth({ refreshDisk = false } = {}) {
    ...
    if (
      !refreshDisk &&
      this.cachedStorage &&
      now - this.cachedStorage.checkedAt < this.storageCheckIntervalMs
    ) {
    ...
    this.cachedStorage = { ...result, checkedAt: now };
    return result;
  }
  ...
  _deleteExpiredDeadLetters({ cutoff, limit = this.cleanupBatchSize } = {}) {
    const safeLimit = Math.max(
      1,
      Math.min(100_000, Number(limit) || this.cleanupBatchSize)
    );
  ...
  cleanupDelivered({ forcePressure = false, reason } = {}) {
    ...
    if (this.deadLetterRetentionMs > 0) {
      const deadCutoff = now - this.deadLetterRetentionMs;
      const deadRows = this._deleteExpiredDeadLetters({
        cutoff: deadCutoff,
        limit: this.cleanupBatchSize,
      });
      deadPurged = deadRows.length;
      this.cleanupStats.lastDeadPurged = deadPurged;
      this.cleanupStats.totalDeadPurged += deadPurged;
      this.cachedStorage = null;
    }
  ...
  recordSinkFailure(
    ...
    const row = this.db
      ...
      .get(sink, now, errorText, limit);
    const control = this._parseSinkControl(row);
  ...
  recordSinkSuccess(sink, now = this.now()) {
    const row = this.db
      ...
      .get(sink, now);
    return this._parseSinkControl(row);
  }
  ...
  /**
   * Delete an exact set of reviewed dead-letter IDs. Missing IDs are reported
   * rather than treated as failures, making retries idempotent.
   * @param {string|string[]} ids
   * @param {{sink?: string}} options
   * @returns {{deleted: number, ids: string[], missing: number, missingIds: string[], sink: string|null}}
   */
  deleteDeadLetters(ids, { sink } = {}) {
    const requestedIds = typeof ids === "string" ? [ids] : ids;
    if (!Array.isArray(requestedIds) || !requestedIds.length) {
      throw new Error("At least one dead-letter ID is required");
    }
    if (requestedIds.length > 10_000) {
      throw new RangeError("At most 10000 dead-letter IDs may be deleted");
    }
    ...
    if (
      ...
      throw new TypeError("Every dead-letter ID must be a non-empty string");
    }

    return this.transaction(() => {
      const remove = sink
        ? this.db.prepare(
            "DELETE FROM dead_letter_jobs WHERE id = ? AND sink = ?"
          )
        : this.db.prepare("DELETE FROM dead_letter_jobs WHERE id = ?");
      const deletedIds = [];
      const missingIds = [];
      for (const id of uniqueIds) {
        const result = sink ? remove.run(id, sink) : remove.run(id);
        if (result.changes === 1) {
          deletedIds.push(id);
        } else {
          missingIds.push(id);
  ...
  stats() {
    ...
    const deadExpired =
      this.deadLetterRetentionMs <= 0
        ...
        : this.db
            ...
            .get(now - this.deadLetterRetentionMs).count;
    ...
    return {
      ...
      deadLettersExpired: deadExpired,
      controls,
      health: {
        ...
        deadLetterRetentionMs: this.deadLetterRetentionMs,
        cleanupHighWatermark: this.cleanupHighWatermark,
        cleanupLowWatermark: this.cleanupLowWatermark,
        protectIngestion: this.protectIngestion,
        cleanup: {
