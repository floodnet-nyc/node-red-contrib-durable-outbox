...
const ERROR_SERIALIZATION_MAX_DEPTH = 8;
const ERROR_SERIALIZATION_MAX_KEYS = 100;
...
function sanitizeErrorValue(value, seen = new Set(), depth = 0) {
  ...
  if (depth >= ERROR_SERIALIZATION_MAX_DEPTH) return "[Maximum depth]";
  ...
  try {
    ...
    for (const key of keys) {
      ...
      result[key] = sanitizeErrorValue(child, seen, depth + 1);
    }

    if (!errorObject && !isPlainObject(value)) {
      const hadProperties = Object.keys(result).length > 0;
      result.type = value.constructor?.name || "unknown";
      if (!hadProperties) result.value = String(value);
    }
    return result;
  } finally {
    seen.delete(value);
...
function serializeError(error) {
  ...
  try {
    ...
    return JSON.stringify({
      name: "ErrorSerializationFailure",
