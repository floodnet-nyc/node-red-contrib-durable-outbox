...
function serializePayload(value) {
  ...
  // Preserve the exact persisted dedup format of `stableStringify`, including
  // its double-JSON-encoding of the canonical-v2 string, so dedup keys stay
  // stable across upgrades.
  const dedupeString = extended
    ? `canonical-v2:${JSON.stringify(payloadJson)}`
